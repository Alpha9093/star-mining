...
const AuthView: React.FC<AuthViewProps> = ({ onSuccess, onCancel }) => {
  ...
  return (
    <div className="flex flex-col items-center py-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <h2 className="text-2xl font-black text-white mb-1">{isLogin ? 'Welcome Back' : 'Create Account'}</h2>
      <p className="text-gray-500 text-[9px] mb-6 uppercase tracking-[0.2em] font-black">Sync your star empire</p>

      <div className="w-full bg-gray-900 border border-gray-800 p-6 rounded-[32px] mb-6 shadow-2xl">
        
        {/* Google Auth Button */}
        <button 
          onClick={handleGoogleSignIn}
          disabled={loading}
          className="w-full py-4 bg-white text-black font-black text-xs uppercase tracking-widest rounded-2xl flex items-center justify-center gap-3 active:scale-95 transition-all shadow-xl disabled:opacity-50 mb-6"
        >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button 
            type="submit"
            disabled={loading}
            className="w-full py-4 bg-gray-800 border border-gray-700 text-white font-black text-xs uppercase tracking-widest rounded-2xl active:scale-95 transition-all disabled:opacity-50"
          >
